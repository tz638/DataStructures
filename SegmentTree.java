        ...
        for (int i = 0; i < 100; ++i)   {
            ...
            for (int a = 0; a < N; ++a)
                ...
                for (int b = a; b < N; ++b)   {
                    ...
                    for (int x = a; x <= b; ++x) min = Math.min(min, list.get(x));

                    int val = list.get(st.minQuery(a, b));

                    if (min != val) System.out.printf("BAD [%d,%d]: correct=%d vs %d\n", a, b, min, val);
                }

            int p = r.nextInt(N), v = r.nextInt();
            st.update(p, v);

            if (list.get(p) != v) System.out.println("UPDATE FAILED!");
        }
    }
}
